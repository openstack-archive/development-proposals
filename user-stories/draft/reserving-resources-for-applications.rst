...
Some Legacy Enterprise Applications require the ability to reserve resources (cpu, memory, storage, networking)
for an expected increase in workload demands. At the time the App is launched it is desired that to specify the 
resources required and that the App is placed on a node with those resources and a hold put on the add’l resources 
so that they aren’t given to another App, during the reservation window. 
The Window is specified in terms of time: 16:00 - 18:00 (example: Earnings Web Cast).  This capability could be more 
important for Legacy apps, because they may not have the ability to add more VM resources once they are launched.
...
Examples of Workloads that may have this type of behavior, include:
1)  Webcast Infrastructure: When the App is launched it needs to reserve enough storage, compute and network bandwidth 
for the expected volume of attendees/viewers.
2) Book, ship, bill apps: Need to have add'l resources at end of month and/or financial close
3) Environments where the cloud is shared and resources for a certain departments applications must have “right of first use” 
of the infrastructure (example could be around a product launch and you want to make sure that the access to the new 
information is prioritized above other non-mission critical information access).
...
An application developer moves a legacy application into the cloud and this application can not 
properly handle running out of resources and will crash in that scenario.  As a cloud administrator 
I need a way to be able to reserve an initial buffer capacity which can not be used by other applications 
running in the cloud, therefore providing this application with a pre-allocated buffer.  This reservation 
must also be extendable in the event the original reserved amount is consumed.
