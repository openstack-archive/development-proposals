...
+++++++++++++++++++++++
**General Lifecycle**
In this process operators are faced with three tasks: detection, trigger and
resolution. The lifecycle moves through the following phases.
 ...
 * Operator performs manual discovery and manual resolution
 * During RCA operator identifies method to automatically detect outage
 * Operator implements automatic detection in auditor system
 * After repeated detection operator describes method for automated remediation
   via code or run-book
 * Operator implements automatic remediation in resolver system
 * Operator implements automatic trigger from auditor to resolver system
