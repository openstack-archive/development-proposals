...
Readable and User-focused Error Messages
========================================
...
Operators currently have to deal with hundreds of potential error messages from the different
OpenStack services, each with varying levels of clarity on where it originated from, what caused
it, and what the potential solution could be.
...
One of the largest barriers to adoption of OpenStack is perceived complexity in managing it after
initial setup ("Day 2 and beyond").
...
* As an app developer or deployer (DevOp), I need to get error messages that clarify which
OpenStack service is generating it, the API call and argument that caused it, and details that
indicate where a potential solution lies (eg. "out of floating IP addresses"). Error messages that
are outside the control of OpenStack (eg. from NIC driver, RabbitMQ etc.) are not part of this

* As an operator, I need error messages with error codes that can be used to refer to a knowledge
base for trouble-shooting and resolution
...
1. App Deployment Error Message
    a. DevOp gets error message when deploying an app or instance
    b. DevOp reviews the source and the details of what caused it
    c. DevOp performs remedial action if error caused by application, else contacts Operator
    d. Operator studies error message, and uses error code to refer to knowledge base (if any) to
    gather information for remedial action
    e. Operator performs remedial action.

2. Operator Error Message
    a. Operator gets error message directly from the system
    b. Operator reviews error details, uses the error code to refer to a knowledge base (if any),
    and determines remedial action
    c. Operator informs tenant (DevOp) of the situation and proposed action
    d. If error caused outside the application's scope, Operator takes remedial action
    e. If error caused by the application, DevOp takes remedial action
...
Common Failure Reporting (https://review.openstack.org/#/c/317695/)
...
UI Alert Guidelines http://docs.openstack.org/contributor-guide/ui-text-guidelines/ui-error-messages.html
...
Cinder spec https://specs.openstack.org/openstack/cinder-specs/specs/newton/summarymessage.html
