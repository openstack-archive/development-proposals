...
Operators currently have to deal with hundreds of potential error messages from the different OpenStack services, each with varying levels of clarity on where it originated from, what caused it, and what the potential solution could be.
...
One of the largest barriers to adoption of OpenStack is perceived complexity in managing it after initial setup ("Day 2 and beyond").
...
* As an app developer or deployer (DevOp), I need to get error messages that clarify which OpenStack service is generating it, the API call and argument that caused it, and details that indicate where a potential solution lies (eg. "out of floating IP addresses").

* As an operator, I need error messages with error codes that can be used to refer to a knowledge base for trouble-shooting and resolution
...
1. App Deployment Error Message 
    ...
    d. Operator studies error message, and uses error code to refer to knowledge base (if any) to gather information for remedial action
    e. Operator performs remedial action.

2. Operator Error Message
    a. Operator gets error message directly from the system
    b. Operator reviews error details, uses the error code to refer to a knowledge base (if any), and determines remedial action
